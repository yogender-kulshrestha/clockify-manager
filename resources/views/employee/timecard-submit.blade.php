                ...
                <div class="card-body px-0 pb-0">
                    <div class="table-responsive p-3">
                        <table class="table table-flush" id="datatable">
                            ...
                            @foreach($rows as $row)
                                <tr>
                                    <td>{{$row->date}}</td>
                                    <td>{{$row->flags}}</td>
                                    <td>{{\Carbon\CarbonInterval::seconds($row->ot_hours)->cascade()->forHumans(['parts' => 1, 'short' => true])}}</td>
                                    <td>{{\Carbon\CarbonInterval::seconds($row->net_hours)->cascade()->forHumans(['parts' => 1, 'short' => true])}}</td>
                        ...
                        <div class="text-center">
                            {{--<form id="submit_form" method="POST" action="{{route('employee.timecard.submit', ['week' => $week])}}">
                            --}}<form action="#">
                                @csrf
                                {{--<input type="hidden" name="start_time" value="{{}}"/>
                                <input type="hidden" name="end_time" value="{{}}"/>--}}
                                <input type="hidden" name="week" value="{{$week}}"/>
                                <input type="submit" id="submit_button" class="btn btn-success btn-sm"/>
                            </form>
                        </div>
                    </div>
                </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            $('#start_time').attr('min', '{{\Carbon\Carbon::parse()->format('Y-m-d\TH:i')}}');
            $('#start_time').attr('max', '{{\Carbon\Carbon::parse()->format('Y-m-d\TH:i')}}');
            $('#end_time').attr('min', '{{\Carbon\Carbon::parse()->format('Y-m-d\TH:i')}}');
            $('#end_time').attr('max', '{{\Carbon\Carbon::parse()->format('Y-m-d\TH:i')}}');
